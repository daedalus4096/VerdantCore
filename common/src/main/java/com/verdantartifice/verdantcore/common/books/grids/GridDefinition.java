package com.verdantartifice.verdantcore.common.books.grids;
...
import com.verdantartifice.verdantcore.common.rewards.ComprehensionReward;
...
public class GridDefinition {
    ...
    public static Codec<GridDefinition> codec() {
        return RecordCodecBuilder.create(instance -> instance.group(
                ...
                ResourceKey.codec(RegistryKeysVC.BOOK_LANGUAGES).fieldOf("language").forGetter(GridDefinition::getLanguage),
    ...
    public static StreamCodec<RegistryFriendlyByteBuf, GridDefinition> streamCodec() {
        return StreamCodec.composite(
                ...
                ResourceKey.streamCodec(RegistryKeysVC.BOOK_LANGUAGES), GridDefinition::getLanguage,
                StreamCodecUtils.VECTOR2I, GridDefinition::getStartPos,
                PlacedNode.streamCodec().apply(ByteBufCodecs.list()).map(nodeList -> {
                    ImmutableMap.Builder<Vector2i, GridNodeDefinition> builder = ImmutableMap.builder();
                    nodeList.forEach(pn -> builder.put(pn.pos(), pn.definition()));
                    return builder.build();
                }, nodeMap -> {
    ...
    public static class Builder {
        ...
        private void validate() {
            ...
            int expected = this.registries.lookupOrThrow(RegistryKeysVC.BOOK_LANGUAGES).getOrThrow(this.language).value().complexity();
            if (total != expected) {
