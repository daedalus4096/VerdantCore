...
public class MainIndexResearchTopic extends AbstractResearchTopic<MainIndexResearchTopic> {
    ...
    public ResearchTopicType<MainIndexResearchTopic> getType() {
        return ResearchTopicTypesVC.MAIN_INDEX.get();
    }
    ...
    public Optional<Component> getUnreadTooltip(Player player) {
        ...
        if (count == 1) {
            return Optional.of(Component.translatable("tooltip.verdantcore.unread_count.discipline.single"));
        } else if (count > 0) {
            return Optional.of(Component.translatable("tooltip.verdantcore.unread_count.discipline.multiple", count));
        } else {
