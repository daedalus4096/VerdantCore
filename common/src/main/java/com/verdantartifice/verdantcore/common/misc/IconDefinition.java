...
public class IconDefinition {
    ...
    public static final StreamCodec<ByteBuf, IconDefinition> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.BOOL, IconDefinition::isItem,
            ByteBufCodecs.BOOL, IconDefinition::isTag,
            ResourceLocation.STREAM_CODEC, IconDefinition::getLocation,
            ByteBufCodecs.optional(ByteBufCodecs.STRING_UTF8), def -> def.tooltipOverrideOpt,
            IconDefinition::new);
    ...
    public static IconDefinition of(ItemLike item) {
        return new IconDefinition(true, false, ServicesVC.ITEMS_REGISTRY.getKey(item.asItem()), Optional.empty());
    }
    ...
    @Nullable
    public Item asItem() {
        return this.isItem ? ServicesVC.ITEMS_REGISTRY.get(this.location) : null;
    }
    ...
    public List<Component> getTooltipLines() {
        if (this.tooltipOverrideOpt.isPresent()) {
            ...
            return ServicesVC.ITEMS_REGISTRY.getTag(this.asTagKey()).stream().map(Item::getDescription).toList();
        } else {
