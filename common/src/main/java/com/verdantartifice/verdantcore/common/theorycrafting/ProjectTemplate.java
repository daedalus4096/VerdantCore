...
import com.verdantartifice.verdantcore.common.research.requirements.QuorumRequirement;
...
public record ProjectTemplate(List<AbstractProjectMaterial<?>> materialOptions, List<AbstractReward<?>> otherRewards, Optional<AbstractRequirement<?>> requirement,
    ...
    public Project initialize(ServerPlayer player, Set<Block> nearby) {
        ...
        ResourceKey<ProjectTemplate> key = registryAccess.registryOrThrow(RegistryKeysVC.PROJECT_TEMPLATES).getResourceKey(this).orElse(null);
        if (key == null) {
        ...
        if (!this.aidBlocks.isEmpty()) {
            boolean found = false;
            Set<ResourceLocation> nearbyIds = nearby.stream().map(b -> ServicesVC.BLOCKS_REGISTRY.getKey(b)).collect(Collectors.toUnmodifiableSet());
    ...
    public static class Builder {
        ...
        public Builder aid(Block block) {
            this.aidBlocks.add(ServicesVC.BLOCKS_REGISTRY.getKey(Preconditions.checkNotNull(block)));
            return this;
        }
