...
public abstract class AbstractWeightFunction<T extends AbstractWeightFunction<T>> {
    public static Codec<AbstractWeightFunction<?>> dispatchCodec() {
        return ServicesVC.WEIGHT_FUNCTION_TYPES_REGISTRY.codec().dispatch("reward_type", AbstractWeightFunction::getType, WeightFunctionType::codec);
    }
    
    public static StreamCodec<RegistryFriendlyByteBuf, AbstractWeightFunction<?>> dispatchStreamCodec() {
        return ServicesVC.WEIGHT_FUNCTION_TYPES_REGISTRY.registryFriendlyStreamCodec().dispatch(AbstractWeightFunction::getType, WeightFunctionType::streamCodec);
    }
