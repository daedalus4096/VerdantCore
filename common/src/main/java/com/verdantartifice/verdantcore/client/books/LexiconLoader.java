...
import com.verdantartifice.verdantcore.common.books.Lexicon;
import com.verdantartifice.verdantcore.common.books.LexiconManager;
...
public class LexiconLoader extends SimpleJsonResourceReloadListener {
    ...
    public void updateWithTagData(RegistryAccess registryAccess) {
        ...
        registryAccess.registryOrThrow(RegistryKeysVC.BOOK_LANGUAGES).holders().forEach(langHolder -> {
            Lexicon lexicon = new Lexicon();
            registryAccess.registryOrThrow(RegistryKeysVC.BOOKS).holders().map(bookHolder -> new BookView(Either.left(bookHolder), langHolder, 0)).forEach(view -> {
